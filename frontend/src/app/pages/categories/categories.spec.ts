...
export class CategoriesComponent {
  ...
  refresh() {
    ...
    this.api.listCategories().subscribe({
      ...
      error: err => {
        ...
        this.snack.open('Failed to load categories', 'Close', { duration: 2500 });
      },
  ...
  add() {
    ...
    if (!n) {
      this.snack.open('Name is required', 'Close', { duration: 2000 });
      return;
    }
    this.loading.set(true);
    ...
    this.api.createCategory(body).subscribe({
      next: _ => {
        this.snack.open('Category added', 'Close', { duration: 1500 });
        this.clearForm();
        this.refresh();
      },
      error: err => {
  ...
  update() {
    ...
    this.api.updateCategory(body).subscribe({
      next: _ => {
        this.snack.open('Category updated', 'Close', { duration: 1500 });
        this.clearForm();
        this.refresh();
      },
      error: err => {
  ...
  remove(row: Category) {
    ...
    this.api.deleteCategory(row.id).subscribe({
      next: _ => {
        this.snack.open('Category deleted', 'Close', { duration: 1500 });
